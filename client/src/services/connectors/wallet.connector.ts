...
class WalletApi {
        ...
        if (utxos) {
            ...
            parsedUtxos.forEach(
                (element: {
                    ...
                    countedValue = countedValue.checked_add(
                        element.output().amount()
                    );
                }
            );
        ...
        if (changeAddress) {
            ...
            const txConfig =
                this.serialLib.TransactionBuilderConfigBuilder.new()
                    ...
                    .key_deposit(
                        this.serialLib.BigNum.from_str(
                            String(protocolParameters.keyDeposit)
                        )
                    )
                    .pool_deposit(
                        this.serialLib.BigNum.from_str(
                            String(protocolParameters.poolDeposit)
                        )
                    )
            ...
            txBuilder.add_output(
                this.serialLib.TransactionOutputBuilder.new()
                    .with_address(
                        this.serialLib.Address.from_bech32(paymentAddress)
                    )
                    .next()
                    .with_value(
                        this.serialLib.Value.new(
                            this.serialLib.BigNum.from_str(adaAmount)
                        )
                    )
                    .build()
            );
            ...
            const utxosFromWalletConnector = (await account.getUtxos()).map(
                (utxo) =>
                    this.serialLib.TransactionUnspentOutput.from_bytes(
                        Buffer.from(utxo, "hex")
                    )
    ...
    async _getProtocolParameter(networkId: number) {
        ...
        return {
            ...
            coinsPerUtxoWord: epochParams.coins_per_utxo_size ? epochParams.coins_per_utxo_size : epochParams.coins_per_utxo_word,
